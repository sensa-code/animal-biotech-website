...
export default function EditTraceabilityPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchRecord = async () => {
    try {
      const res = await fetch(`/api/admin/traceability/${resolvedParams.id}`)
      const data = await res.json()

      if (data.success && data.data) {
        const record: ProductRecord = data.data
        setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        toast({
          title: '更新成功',
          description: `已更新產品編碼「${formData.product_code}」`,
        })
        router.push('/admin/traceability')
      } else {
        toast({
          title: '更新失敗',
          description: data.message || '無法更新記錄',
          variant: 'destructive',
        })
      }
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="p-8 flex items-center justify-center min-h-[400px]">
        <Loader2 className="w-8 h-8 animate-spin text-[oklch(0.55_0.01_240)]" />
      </div>
    )
  }
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-[oklch(0.95_0.01_90)]">編輯溯源記錄</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input
              ...
              required
              className="w-full px-4 py-2.5 rounded-lg bg-[oklch(0.16_0.01_240)] border border-[oklch(0.25_0.02_240)] text-[oklch(0.95_0.01_90)] focus:border-[oklch(0.70_0.08_160)] focus:outline-none font-mono"
            />
          </div>
